...
@interface XHImageViewer () <IBActionSheetDelegate> {
...
- (void)showWithSelectedView:(UIImageView *)selectedView {
	...
	[UIView animateWithDuration:0.3
	    ...
	    UILongPressGestureRecognizer *longGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longTappedScrollView:)];
...
- (void)longTappedScrollView:(UITapGestureRecognizer *)sender {
	if (!_sheet.superview) {
		_sheet = [[IBActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"保存圖片", nil];
		[_sheet showInView:[[[UIApplication sharedApplication] windows] firstObject]];
	}
}

- (void)actionSheet:(IBActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	if (buttonIndex == 0) {
		UIImage *savedImage = [[self.imgViews objectAtIndex:self.scrollView.contentOffset.x / [[UIScreen mainScreen] bounds].size.width] image];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
	if (error != NULL) {
		[MRProgressOverlayView showNO:@"保存圖片失敗，請檢查設置是否允許訪問圖片！"];
	}
	else {
		[MRProgressOverlayView showYES:@"保存圖片成功！"];
	}
}
