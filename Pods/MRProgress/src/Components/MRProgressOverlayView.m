...
+ (instancetype)showYES:(NSString *)title {
	if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0) {
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
		    [SVProgressHUD showSuccessWithStatus:title];
		});

		return [[MRProgressOverlayView alloc] init];
	}
	else {
...
+ (instancetype)showNO:(NSString *)title {
	if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0) {
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
+ (instancetype)showLoading:(NSString *)title {
	if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0) {
...
- (void)show:(BOOL)animated {
	...
	if (animated) {
		[self setSubviewTransform:CGAffineTransformMakeScale(0.8f, 0.8f) alpha:0.5f];
