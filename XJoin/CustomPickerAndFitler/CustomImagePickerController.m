...
@interface CustomImagePickerController () <ImageFitlerProcessDelegate>
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
	...
	if (version >= 5.0) {
		...
		[navigationController.navigationBar insertSubview:[[[UIImageView alloc] initWithImage:backgroundImage] autorelease] atIndex:1];
	}
...
- (void)    imagePickerController:(UIImagePickerController *)picker
	...
	image = [[image clipImageWithScaleWithsize:CGSizeMake(image.size.width/8, image.size.height/8)] retain];
	[[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleBlackOpaque animated:YES];
	if ([_customDelegate respondsToSelector:@selector(cameraPhoto:)]) {
		[_customDelegate cameraPhoto:image];
	}

	ImageFilterProcessViewController *fitler = [[[ImageFilterProcessViewController alloc] init] autorelease];
	[fitler setDelegate:self];
	fitler.currentImage = image;
	//	    UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:fitler];
	//	    nav.navigationBar.barTintColor = COLOR_MAIN_RED_DARK;
	//	    nav.navigationBar.translucent = NO;
	//	    nav.navigationBar.titleTextAttributes = @{ UITextAttributeTextColor:[UIColor whiteColor] };
	//	    [self presentModalViewController:nav animated:YES];
	[self pushViewController:fitler animated:YES];
}

- (void)imageFitlerProcessDone:(UIImage *)image {
	if ([self.customDelegate respondsToSelector:@selector(pickedPhoto:)]) {
		[self.customDelegate pickedPhoto:image];
	}
}
