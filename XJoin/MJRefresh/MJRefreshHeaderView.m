...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.pullToRefreshText = NSLocalizedString(@"下拉可以刷新",nil);
        self.releaseToRefreshText = NSLocalizedString(@"鬆開立即刷新",nil);
        self.refreshingText = NSLocalizedString(@"正在刷新...",nil);
    }
    return self;
}
...
- (void)updateTimeLabel
{
    ...
    if ([cmp1 day] == [cmp2 day]) { // 今天
        formatter.dateFormat = NSLocalizedString(@"今天 HH:mm",nil);
    ...
    // 3.显示日期
    self.lastUpdateTimeLabel.text = [NSString stringWithFormat:NSLocalizedString(@"最后更新：%@",nil), time];
}
